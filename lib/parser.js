...
_.extendOwn(MailParser.prototype, {
    parseTaskLetter: function (mail) {
        try {
            var letter = mail.data,
                parsed = extractMarkers(textToLines(
            ...
            this.emit('error', exc, mail);
            return;
        }
        this.emit('task', {markers: markers, task: task, lines: lines}, mail);
    },

    parseReplyLetter: function (mail, token) {
        try {
            var letter = mail.data,
                parsed = extractMarkers(cropLines(textToLines(
        ...
        this.emit('reply', {markers: markers, reply: reply, lines: lines}, mail);
    }
});
