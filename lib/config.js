            ...
            res.on('end', function () {
                ...
                if (res.statusCode === 200) {
                    try {
                        ...
                        json
                            ? log.err('Configuration text:', JSON.stringify(json, null, 4))
                            : (bytes && bytes.toString('utf8') && log.err('Configuration text:', bytes.toString('utf8')));
                        process.exit(1);
                        return;
                    }
                    resolve(data);
...
function build(data) {
    return check({
        ...
        compose: {
            timeout: data.compose && data.compose['timeout'],
            dataKey: data.compose && data.compose['data-key'],
            composeEvent: data.compose && data.compose['compose-event'],
            templateNames: {
                usermailTaskError: data.compose && data.compose['template-names'] && data.compose['template-names']['usermail-task-error'],
                usermailReplyError: data.compose && data.compose['template-names'] && data.compose['template-names']['usermail-reply-error']
            }
        }
    }, {
        redis: {
        ...
        compose: {
            ...
            dataKey: required(),
            composeEvent: required(),
            templateNames: {
                usermailTaskError: required(),
                usermailReplyError: required()
            }
        }
    });
}
