...
_.extendOwn(Backend.prototype, {
    alterTask: function (model) {
        return this.request({
            method: 'POST',
            uri: 'api/email/reply?key=' + this.key,
            form: model
        });
    },
    createTask: function (model) {
        return this.request({
            method: 'POST',
            uri: 'api/email/task?key=' + this.key,
            form: model
        });
    },
    user: function (email) {
        return this.request({
            method: 'GET',
            uri: 'api/email/user?key=' + this.key + '&email=' + encodeURIComponent(email)
        });
    },
        ...
        return $q(function (resolve, reject) {
            try {
                var method = options.method || 'GET',
                    url_ = url.resolve(this.url, options.uri),
                    form = options.form || {};

                log.out('%s %s', method, url_, '\n' + stringify(form, null, 4));

                needle.request(method, url_, form, {json: true}, function (err, resp, body) {
                    var result;
                    try {
                        if (err) {
